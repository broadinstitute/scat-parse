...
| Member | Meaning |
| --- | --- |
| `Token` | one token (`Char` for the string instance). May box — the hot path never touches it. |
...
- **Hot core** — `length`, `matchesAt`, `scanWhile`, `startsWithAt`, `slice`. Called on every parse
  step, so they take and return primitives, `S`, or `Slice` — never `Token`. Bulk operations such as
...
- **Cold error section** — `expectSet`, `mergeOfAlphabet`, `orderingS`, `orderOfAlphabet`,
  `showOfAlphabet`, `showLiteral`, `renderContext`, `tokenAt`, `subInput`. Reached only when a parse
  error is built or rendered.
...
object CodeAlphabet extends Alphabet[Vector[Int]] {
  ...
  /** The whole token domain. `setWhere` below enumerates it, so it has to be finite. */
...
val run: Parser[Vector[Int], Vector[Int]] = Parser.tokensWhile(CodeAlphabet)(_ < 5)
...
run.parse(Vector(1, 2, 3, 7, 0))

opcode.parse(Vector(1, 2, 3, 9))

(Parser.seq(Vector(1, 2)) ~ run).parse(Vector(1, 2, 0, 4, 8))
```
